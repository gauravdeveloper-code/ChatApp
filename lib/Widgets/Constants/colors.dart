import 'dart:ui';

const backgroundColor = Color.fromRGBO(13, 20, 24,1);
const textColor = Color.fromRGBO(241, 241, 242, 1);
const appBarColor = Color.fromRGBO(42,47,50,1);
const messageColor2 = Color.fromRGBO(18, 140, 126, 1);
const messageColor = Color.fromRGBO(7, 94, 84, 1);
const searchBarColor = Color.fromRGBO(50, 55, 57, 1);
const dividerColor = Color.fromARGB(22, 30, 50, 2);
const chatBoxColor = Color.fromARGB(54, 57, 57, 3);